...
resource "null_resource" "build_ecr_image" {
  ...
  provisioner "local-exec" {
    command = <<EOF
        ...
        NC='\033[0m' # No Color
        if [ "$STATUS" -eq 0 ]; then
          printf "\n $GREEN SERVICE Docker Image Successfull: $1  $NC \n"
        else
          printf "\n $RED SERVICE Docker Image failed: $1 $NC \n" >&2
          exit $STATUS
        fi
      }
          cd ../${path.root}
          ...
          docker buildx build --platform linux/amd64 -t ${aws_ecr_repository.airflow.repository_url}:latest -f airflow_services/Dockerfile .
          exit_status "build"
  ...
  provisioner "local-exec" {
    command = <<EOF
        ...
        if [ "$STATUS" -eq 0 ]; then
          ...
          cd ../${path.root}
          aws ecr get-login-password --region ${var.aws_region} | docker login --username AWS --password-stdin ${var.account_id}.dkr.ecr.${var.aws_region}.amazonaws.com
          docker buildx build --platform linux/amd64 -t ${aws_ecr_repository.worker_airflow.repository_url}:latest -f airflow_worker/Dockerfile .
          exit_status "build"
