...
def get_unique_hostnames_for_states(states: List[str]) -> List[str]:
    ...
    with session_scope(Session) as _session:
        unique_hostnames_query = (
            _session.query(TaskInstance.hostname.distinct())
...
def get_tasks_info(ecs_client, cluster_name, service_name):
    ...
    tasks_info = ecs_client.describe_tasks(
        cluster=cluster_name,
        tasks=running_task_arns
    ) if running_task_arns else {'tasks': []}
...
def scale_down_ecs_service(ecs_client, cluster_name, service_name, task_state):
    tasks_to_kill = set()
    tasks_to_not_kill = set()
    # Get tasks info
    tasks_info = get_tasks_info(ecs_client=ecs_client, cluster_name=cluster_name, service_name=service_name)
    ...
    new_desired_count = max(0, current_desired_count - len(tasks_to_kill))
    ...
    print(f"ECS service {service_name} scaled down to {new_desired_count} tasks.")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        metric_value = get_capacity_provider_reservation(current_task_count=task_count,
                                                         current_worker_count=worker_count,
                                                         desired_tasks_per_instance=args.desired_count)
        if metric_value > 100:
            logging.info(f"We are scaling up {metric_value}")
            scale_up_ecs_service(ecs_client=ecs, cluster_name=args.cluster_name,
                                 service_name=args.worker_service_name, max_desired_count=args.desired_count)

        elif metric_value < 100:
            scale_down_ecs_service(ecs_client=ecs, cluster_name=args.cluster_name,
                                   service_name=args.worker_service_name, task_state=[State.RUNNING])
